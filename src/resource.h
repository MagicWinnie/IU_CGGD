	...
	struct unsigned_color
	{
		static unsigned_color from_color(const color& color)
		{
			return unsigned_color{
					static_cast<uint8_t>(std::clamp(static_cast<int>(color.r * 255.f), 0, 255)),
					static_cast<uint8_t>(std::clamp(static_cast<int>(color.g * 255.f), 0, 255)),
					static_cast<uint8_t>(std::clamp(static_cast<int>(color.b * 255.f), 0, 255)),
			};
		};
		static unsigned_color from_float3(const float3& color)
		{
			float3 pre = clamp(255.f * color, 0.f, 255.f);
			return unsigned_color{static_cast<uint8_t>(pre.x), static_cast<uint8_t>(pre.y), static_cast<uint8_t>(pre.z)};
		};
		float3 to_float3() const
		{
